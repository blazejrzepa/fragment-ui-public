...
export async function processMarkdownContent(markdownContent: string) {
  // Normalize input (BOM / zero-width chars can break frontmatter detection)
  const normalizedMarkdown = markdownContent.replace(/^[\uFEFF\u200B\u200C\u200D\u2060]/, "");

  // Fallback frontmatter parser in case gray-matter doesn't detect it for some reason.
  // If frontmatter isn't stripped, markdown can interpret:
  //   title: ...
  //   description: ...
  //   ---
  // as a Setext heading (h2), which then leaks into rendered content.
  const frontmatterBlockMatch = normalizedMarkdown.match(
    /^---\s*\r?\n([\s\S]*?)\r?\n---\s*\r?\n?/
  );
  ...
  // Parse frontmatter
  let { data, content } = matter(normalizedMarkdown);

  // Manual fallback: if gray-matter didn't extract anything but we do see a frontmatter block,
  // strip it and parse simple `key: value` pairs.
  if ((!data || Object.keys(data).length === 0) && frontmatterBlockMatch) {
    const rawFrontmatter = frontmatterBlockMatch[1];
    ...
    data = parsed;
    content = normalizedMarkdown.slice(frontmatterBlockMatch[0].length);
  }
